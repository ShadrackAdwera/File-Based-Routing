...
const commentsHandler = async (req, res) => {
  const client = await MongoClient.connect(
    'mongodb+srv://next_party_user:kawz8hoI89Whafwp@cluster0.dska4.mongodb.net/next-party?retryWrites=true&w=majority'
  );
  ...
  if (req.method === 'POST') {
    ...
    try {
      await dbConnected
        .collection('comments')
        .insertOne({ name, email, comment: text, eventId });
